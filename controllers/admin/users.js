...
const allUsers = async (req, res) => {
  try {
    ...
    const jobsData = await jobs.countDocuments({ vendorId: { $in: emails } });
    const leadsData = await Lead.countDocuments({ vendorId: { $in: emails } });
    const projectsData = await Project.countDocuments({
      vendorId: { $in: emails },
    });
    res.status(200).json({
      users: users,
